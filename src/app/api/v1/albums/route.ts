...
/**
 * `/api/v1/albums` — Albums collection endpoint (PRD §21).
 */
export async function GET(request: NextRequest) {
  ...
  const collection = await getCollectionPage('albums', {
    page,
    pageSize,
    search,
  })
  ...
  if (!collection) {
    return errorResponse('Albums collection not found', 'NOT_FOUND', 404)
  }
