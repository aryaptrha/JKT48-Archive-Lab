...
/**
 * `/api/v1/generations` — Generations collection endpoint (PRD §21).
 */
export async function GET(request: NextRequest) {
  ...
  const collection = await getCollectionPage('generations', {
    page,
    pageSize,
    search,
  })
  ...
  if (!collection) {
    return errorResponse('Generations collection not found', 'NOT_FOUND', 404)
  }
