...
/**
 * `/api/v1/members` — Members collection endpoint (PRD §21).
 */
export async function GET(request: NextRequest) {
  ...
  const collection = await getCollectionPage('members', {
    page,
    pageSize,
    search,
  })
  ...
  if (!collection) {
    return errorResponse('Members collection not found', 'NOT_FOUND', 404)
  }
