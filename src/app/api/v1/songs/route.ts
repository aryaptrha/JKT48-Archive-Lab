...
/**
 * `/api/v1/songs` — Songs collection endpoint (PRD §21).
 */
export async function GET(request: NextRequest) {
  ...
  const collection = await getCollectionPage('songs', {
    page,
    pageSize,
    search,
  })
  ...
  if (!collection) {
    return errorResponse('Songs collection not found', 'NOT_FOUND', 404)
  }
